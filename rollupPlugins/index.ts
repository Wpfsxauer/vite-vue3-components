 ...
 * @Description:
 * @Autor: weipengfei
 * @Date: 2021-07-26 18:23:02
 * @LastEditors: weipengfei
 * @LastEditTime: 2021-07-27 17:25:07
 */
        ...
        async closeBundle() {
            ...
            await runParallel(
                ...
                async (outputFile) => {
                    ...
                    if (packagesReg.test(filePath)) filePath = filePath.replace(packagesReg, "");

                    const srcReg = /src\//g;
                    if (srcReg.test(filePath)) filePath = filePath.replace(srcReg, "");

                    // if (!filePath.includes("index.vue.d")) {
                        ...
                        entries
                            .map((entry) => {
                                let filePath = normalizePath(
                                    relative(
                                        dirname(typesPath),
                                        resolve(outputDir, relative(root, entry))
                                    )
                                );
